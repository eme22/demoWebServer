...
<body>
<% String user = new String(Base64.getDecoder().decode(request.getParameter("sessioncode"))); %>
...
<script type="text/javascript">
    ...
    $(document).ready(function ()  {
        addmodal = new bootstrap.Modal(document.getElementById('exampleModal'));
        ...
        $.ajax({
            ...
            success: function(xml) {
                $(xml).find('return').each(function(){
                    var contrasenia = $(this).find('contrasenia').text();
                    ...
                    $("#userstable").find('tbody').append(
                        ...
                        usuario +
                        "</td>" +
                        "<td id=\"pw@"+id+"\" class=\"hidetext\">" +
                        contrasenia +
                        "</td>" +
                        "<td>" +
                        "<button id=\"edit@"+id+"\" type=\"button\" class=\"btn btn-primary tablebtn\">Editar</button>" +
                        "<button id=\"erase@"+id+"\" type=\"button\" class=\"btn btn-secondary tablebtn\">Borrar</button>" +
                        "<button id=\"show@"+id+"\" type=\"button\" class=\"btn btn-light tablebtn\">Mostrar Contrasenia</button>" +
                        "</td>" +
                        "</tr>");
                });
                var botones = document.getElementsByClassName("tablebtn");

                function buttonfunc() {
                    var actionandid = this.getAttribute("id").split("@");
                    switch (actionandid[0]) {
                        case "edit": editrow(actionandid[1]);break;
                        case "erase": deleterow(actionandid[1]); break;
                        case "show": showrow(actionandid[1]);break;
                    }
                }

                Array.from(botones).forEach(function(botones) {
                    botones.addEventListener('click', buttonfunc);
                });
            },
            error: loadError
        });
    ...
    function editrow(string) {
        if (string){
            const wsUrl = "http://localhost:8080/demoWebServer-1.0-SNAPSHOT/usuario?wsdl";
            const soapRequest =
                "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:dem=\"http://demoWebServer.eme22.com/\">" +
                "<soapenv:Header/>" +
                "<soapenv:Body>" +
                "<dem:select>" +
                "<arg0>" +
                document.getElementById("usr@" + string).innerText +
                "</arg0>" +
                "</dem:select>" +
                "</soapenv:Body>" +
                "</soapenv:Envelope>";
            $.ajax({
                ...
                success: function(xml) {
                    $(xml).find('return').each(function (){
                        ...
                        $("#exampleInputPassword1").val(contrasenia);
                    });
                },
                error: loadError
            });
    ...
    function loadError(data, status, req) {
    ...
    function insert(){
        var usr = $("#exampleInputEmail1").val();
...
<header class="page-header">
    ...
    <nav>
        <a href="#0" aria-label="forecastr logo" class="logo">
            <img height="49" src="./assets/img/logo-utp.png" alt="UTP logo">
            </svg>
        </a>
...
<section class="page-content">
    <section class="search-and-user">
        <!-- Form Busqueda -->
        <form>
            <input type="search" placeholder="Buscar..." id="usersearch">
            <button type="submit" aria-label="submit form" onclick="search()">
                <svg aria-hidden="true">
        ...
        <div class="admin-profile">
            <span class="greeting username" id="username2"><%=user%></span>
            ...
            <div class="auth-details">
                ...
                <ul>
                    <li>
                        <a class="btn-option" href="${pageContext.request.contextPath}/">Cerrar Sesión</a>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button type="button" class="btn-close" data-bs-dismiss="modal" onclick="clearpopup()" aria-label="Close"></button>
                </div>
                    ...
                    <div class="container">
                        <div class="row justify-content-md-center">
                            ...
                            <div class="mb-3">
                                ...
                                <div id="emailHelp" class="form-text">No le diremos tu email a nadie.</div>
                            </div>
                            <div class="mb-3">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" onclick="clearpopup()">Cancelar</button>
    ...
    <section class="grid">
        <!-- Articulos -->
        <article>
            <button type="button" class="btn btn-primary" onclick="editrow(null)">Agregar Usuario</button>
    ...
    <footer class="page-footer">
        ...
        <a href="https://georgemartsoukos.com/" target="_blank">
            <img height="24" src="./assets/img/logo-utp.png" alt="UTP logo">
        </a>
    </footer>
</section>
